...
def context(lines, idx, radius=6):
    lo = max(0, idx - radius)
...
def main():
    ...
    rows = []
    contexts = []
    important = {"taskalignedassigner", "assigner", "one2one", "one2many", "fg_mask", "target_scores", "bbox_loss", "cls_loss", "dfl_loss", "topk", "top_k"}
    ...
    with (out_dir / "loss_assigner_keyword_hits.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=["file", "line", "keyword", "text"])
        w.writeheader()
        w.writerows(rows)

    md = ["# Week 17 Loss / Assigner / no-NMS Source Audit", ""]
    for c in contexts:
        md += [f"## {c['file']}:{c['line']} — {c['keyword']}", "```python", c["context"].rstrip(), "```", ""]
    (out_dir / "loss_assigner_contexts.md").write_text("\n".join(md), encoding="utf-8")
    ...
    (out_dir / "loss_assigner_audit_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
