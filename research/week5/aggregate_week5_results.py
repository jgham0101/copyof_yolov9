...
def normalize_e2e_metrics(path: Path, model: str, postprocess: str) -> dict:
...
def main():
    ...
    baseline_log = drive / "logs" / "week5_baseline_val_dual.log"
    rows.append(parse_val_dual_log(baseline_log))

    no_nms_metrics = find_one(repo, "week5_proposed_no_nms*/metrics.json")
    nms_metrics = find_one(repo, "week5_proposed_nms*/metrics.json")

    if no_nms_metrics:
        rows.append(normalize_e2e_metrics(no_nms_metrics, "proposed_v10dual", "no-nms"))
    if nms_metrics:
        rows.append(normalize_e2e_metrics(nms_metrics, "proposed_v10dual", "nms"))
