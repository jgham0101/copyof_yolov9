...
def copy_branch_weights(model):
    ...
    for src_name, dst_name in [('cv2','one2one_cv2'), ('cv3','one2one_cv3'), ('dfl','dfl2')]:
        if not hasattr(head, src_name) or not hasattr(head, dst_name):
            report['errors'].append(f'missing {src_name} or {dst_name}')
            continue
        src, dst = getattr(head, src_name), getattr(head, dst_name)
        try:
...
def main():
    ...
    model = Model(args.cfg, ch=3, nc=nc, anchors=None).float()
    ckpt = torch_load(args.source_weights)
    src_model = get_model(ckpt)
    ...
    torch.save({'epoch': -1, 'best_fitness': None, 'model': copy.deepcopy(model).half(), 'ema': None, 'updates': None, 'optimizer': None, 'week15_note': 'compatible transfer + one2many->one2one branch copy'}, out)
    report = {'source_weights': args.source_weights, 'out_weights': str(out), 'out_weights_size': out.stat().st_size, 'source_state_items': len(src_state), 'target_state_items': len(dst_state), 'transferred_items': len(compatible), 'branch_copy_report': branch_report}
